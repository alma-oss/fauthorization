...
module Authorize =
    ...
    let private isGranted softwareComponent keyForRenewToken keysForToken permission (SecurityToken token) =
        permission
        |> JWTToken.isGranted softwareComponent keysForToken token
        <!> (JWTToken.renew keyForRenewToken >> RenewedToken)

    type Authorize<'RequestData> = Lmc.SC.DomainModel.SoftwareComponent -> KeyForRenewToken -> JWTKey list -> SecureRequest<'RequestData> -> Result<RenewedToken * 'RequestData, AuthorizationError>

    let withLogin: Authorize<'RequestData> =
        fun softwareComponent keyForRenewToken keysForToken ->
            SecureRequest.accessData (isGranted softwareComponent keyForRenewToken keysForToken ValidToken)

    let private grantScope softwareComponent keyForRenewToken keysForToken token scope =
        let permission =
            scope
        ...
        token
        |> isGranted softwareComponent keyForRenewToken keysForToken permission

    let withScope scope: Authorize<'RequestData> =
        fun softwareComponent keyForRenewToken keysForToken ->
            SecureRequest.accessData (fun token ->
                scope |> grantScope softwareComponent keyForRenewToken keysForToken token
            )

    let withScopeByRequest (scopeFromRequest: 'RequestData -> Scope): Authorize<'RequestData> =
        fun softwareComponent keyForRenewToken keysForToken ->
            SecureRequest.access (fun token ->
                scopeFromRequest >> grantScope softwareComponent keyForRenewToken keysForToken token
            )
    ...
    let withScopeResultFromRequest (scopeResultFromRequest: 'RequestData -> Result<Scope, ErrorMessage>): Authorize<'RequestData> =
        fun softwareComponent keyForRenewToken keysForToken ->
            SecureRequest.access (fun token ->
                ...
                >=> (grantScope softwareComponent keyForRenewToken keysForToken token)
            )
    ...
    let authorizeAction
        ...
        = asyncResult {
            let! (renewedToken, requestData) =
                request
                |> authorize softwareComponent keyForRenewToken keysForToken
